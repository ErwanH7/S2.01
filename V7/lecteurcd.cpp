...
void LecteurCD::precedent()
{
    ...
    case LECTURE:
        etat = LECTURE;
        titreEnCours = leTiroirCD.getLeCD()->getTitres() -1;
        rangTitreEnCours = -1;
        laCellule.setSource(QString::fromStdString(titreEnCours->getUrl()));
        laCellule.allerADebutMedia();
        laCellule.demarrerLecture();
        qDebug() << "[LecteurCD] precedent() → Titre précédent, rang = " << getRangTitreEnCours();
        break;
    case CHARGE_ARRET:
        etat = PAUSE;
        titreEnCours = leTiroirCD.getLeCD()->getTitres();
        ...
        break;
    case PAUSE:
        if(!(getRangTitreEnCours()==2 && laCellule.positionTeteDeLecture()==0)) {
            etat = PAUSE;
            titreEnCours = leTiroirCD.getLeCD()->getTitres() -1;
            rangTitreEnCours = -1;
            laCellule.setSource(QString::fromStdString(titreEnCours->getUrl()));
...
void LecteurCD::suivant()
{
    ...
    case LECTURE:
        ...
        laCellule.allerADebutMedia();
        laCellule.demarrerLecture();
        qDebug() << "[LecteurCD] suivant() → Titre suivant, rang = " << getRangTitreEnCours();
        break;
    case CHARGE_ARRET:
        etat = PAUSE;
        titreEnCours = leTiroirCD.getLeCD()->getTitres();
        ...
        break;
    case PAUSE:
        if(!(getRangTitreEnCours()==4 && laCellule.positionTeteDeLecture()==0)) { //car 4 titres au total
            etat = PAUSE;
            titreEnCours = leTiroirCD.getLeCD()->getTitres();
            rangTitreEnCours = 0;
            laCellule.setSource(QString::fromStdString(titreEnCours->getUrl()));
...
void LecteurCD::debut()
{
    ...
    case LECTURE:
    {
        ...
        getVue()->getUi()->labelIntitule->setText("RossBugden-Notturno");

        int minutes = leTiroirCD.getLeCD()->getTitre(1).getDuree() / 60;
        int secondes = leTiroirCD.getLeCD()->getTitre(1).getDuree() % 60;
        QString dureeFormatee = QString::asprintf("%02d:%02d", minutes, secondes);
        getVue()->getUi()->labelTempsFin->setText("/ " + dureeFormatee);
        getVue()->getUi()->labelTempDebut->setText("00.00");
        getVue()->getUi()->tempEcoule->setValue(0);

        getVue()->getUi()->statusBar->showMessage("CHARGE_ARRET");
    ...
    case PAUSE:
        if (!(getRangTitreEnCours()==1)){
            ...
            getVue()->getUi()->statusBar->showMessage("PAUSE");
            qDebug() << "[LecteurCD] debut() → Reprise au début du titre courant";
        }
        if (getRangTitreEnCours()==1){
            etat = CHARGE_ARRET;
...
void LecteurCD::modeBoucle()
{
    switch (mode) {
    case BOUCLE:
        // Si le mode est déjà BOUCLE, on revient à SEQUENTIEL (désactivation)
        setMode(SEQUENTIEL);
        qDebug() << "[LecteurCD] modeBoucle() → Mode BOUCLE désactivé, retour à SEQUENTIEL";
        break;

    default:
        // Sinon, on active le mode BOUCLE
        setMode(BOUCLE);
        qDebug() << "[LecteurCD] modeBoucle() → Mode BOUCLE activé";
        break;
    }
...
void LecteurCD::modeSequentiel()
{
    switch (mode) {
    case SEQUENTIEL:
        // Déjà en mode SEQUENTIEL, rien à faire
        qDebug() << "[LecteurCD] modeSequentiel() → Mode SEQUENTIEL déjà actif";
        break;

    default:
        // Passage explicite au mode SEQUENTIEL
        setMode(SEQUENTIEL);
        qDebug() << "[LecteurCD] modeSequentiel() → Passage en mode SEQUENTIEL";
        break;
    }
}
